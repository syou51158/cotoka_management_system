...
try {
    $rpc = supabaseRpcCall('services_list_by_salon', ['p_salon_id' => $salon_id]);
    if (!$rpc['success']) {
        http_response_code(500);
        echo json_encode(['error' => 'Supabaseエラー: ' . ($rpc['message'] ?? '')]);
        exit;
    }
    $services = is_array($rpc['data']) ? $rpc['data'] : [];
    echo json_encode($services);
} catch (Exception $e) {
    error_log('サービスリスト取得エラー: ' . $e->getMessage());
    http_response_code(500);
